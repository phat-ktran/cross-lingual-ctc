...
from losses.softctc.models import BatchConnections


class SoftCTCLossFunction(torch.autograd.Function):
    ...
    def __call__(self, logits, connections: BatchConnections, labels):
        return self.apply(
            logits, connections, labels, self._norm_step, self._zero_infinity
        )
    ...
    def forward(
        ...
        for t in range(1, T):
            current_vector = (
                torch.bmm(
                    current_vector.reshape(N, 1, -1), connections.forward
                ).reshape(N, -1)
                * probs[:, :, t]
            )
        ...
        if zero_infinity:
            zero_mask = torch.logical_or(
                torch.isnan(ll_forward), torch.isinf(ll_forward)
            )
    ...
    def backward(ctx, ll_forward):
        ...
        for t in range(T - 2, -1, -1):
            current_vector = (
                torch.bmm(
                    current_vector.reshape(N, 1, -1), connections.backward
                ).reshape(N, -1)
                * probs[:, :, t]
            )
...
class SoftCTCLoss(torch.nn.Module):
    def __init__(self, norm_step=10, zero_infinity=True):
    ...
    def forward(self, logits, batch):
        ...
        for index, l in enumerate(labels):
            ...
            labels[index] = torch.concat(
                [
                    torch.tensor(labels[index], dtype=torch.int64),
                    torch.tensor(labels_padding, dtype=torch.int64),
                ]
            )
        labels = torch.stack(labels).to(logits.device)
        logits = logits.permute((0, 2, 1))  # B, V, T
        batch_connections = batch_connections.torch().to(logits.device)
